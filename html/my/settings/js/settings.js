        ...
        init: function() {
            ...
            apps.axget(
                ...
                function(data) {
                    ...
                    if(vm.shopselect.birthday.length > 0){
                      document.getElementById("birthdayDiv").onclick = null;
                    }
                });
        },
        //上传图片附件
...
apiready = function() {
    ...
    new Mdate("birthdayDiv",{
      //此项为你要显示选择后的日期的input，不填写默认为上一行的"dataSelect"
      acceptId: "birthdayInfo",
...
function submitBirthday() {
  api.confirm({
    ...
    var index = ret.buttonIndex;
    if(index == 1){
      vm.shopselect.birthday = $("#birthdayInfo").val();
      apps.axpost(
        "customer/updateInfoBirthday", {
          //需要提交的参数值
          birthday:vm.shopselect.birthday
        },
        function(data) {
          vm.init();
          document.getElementById("birthdayDiv").onclick = null;
