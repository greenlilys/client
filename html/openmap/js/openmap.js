...
var vm = new Vue({
    ...
    methods: {
        ...
        init: function() {
            ...
            //加载地图
            if(vm.setBmap.lon && vm.setBmap.lat){
              vm.openbmp();
            };

            setTimeout(function(){
              var notice = $api.getStorage("notices");
              if (notice === undefined) {
                vm.popNotices();
                notice = 1;
                $api.setStorage("notices",notice);
              }
            },2000);
            // 获取店铺列表数据
            ...
            //获取生日活动是否开启和生日信息
            apps.axget(
              "customer/selectisBirthdayOpen", {},
              function(data) {
                if (data) {
                  vm.birthdayInfo = data;
                  var currentDate = new Date();
                  var date = new Date(vm.birthdayInfo.birthday);
                  if(vm.birthdayInfo.isbirthdayopen == 1 && (currentDate.getMonth() == date.getMonth() && currentDate.getDate() == date.getDate()) ){
                    api.openFrame({
                      name: 'birthdayAct-frm',
                      url: 'birthdayAct_frm.html',
                      rect: {
                        x: 0,
                        y: 68,
                        w: 'auto',
        ...
        setmapIcons: function(lon, lat) {
            setTimeout(function() {
                ...
                if ((vm.ShopAndServicedata.isdeposit === 0 && vm.ShopAndServicedata.isabnormal === 0) || vm.ShopAndServicedata.isoverdue === 1) {
                  //逾期退租提示框
                  if(vm.ShopAndServicedata.bespeak && vm.ShopAndServicedata.bespeak.type == 2){
                    api.openFrame({
