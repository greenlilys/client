...
var vm = new Vue({
    ...
    methods: {
        ...
        init: function() {
            ...
            //api.pageParam.ShopAndServicedata.bespeak == '' 没有预约
            if(api.pageParam.ShopAndServicedata.bespeak){
              vm.state = api.pageParam.ShopAndServicedata.bespeak.state;
            }
            // console.log(JSON.stringify(vm.ShopAndServicedata));
                ...
                if (index == 1) {
                    apps.axpost(
                        "batteryService/undoService", {
                            id: vm.ShopAndServicedata.bespeak.id
                        },
                        function(data) {
                            api.toast({
                                msg: '撤销预约成功',
...
function kiDateendtime() {
    ...
    vm.afterMinTimes =  hour + '小时' + min + '分' + s + '秒'
    if (t <= 0) {
